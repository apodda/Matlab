...
lhs = (id_mat - 0.5 * k / (h^2) * laplacian);
rhs = (id_mat + 0.5 * k / (h^2) * laplacian);

lhs_pruned = lhs(not(pattern), not(pattern));
rhs_pruned = rhs(not(pattern), not(pattern));

constant_term = k / (h^2) * laplacian(not(pattern), pattern) * initial_conditions_vector(pattern);
...
for i=2:length(time_vector)
  ...
  %U_pruned(:, i) = k * b * laplacian(not(pattern), not(pattern))* U_pruned(:, i-1) + U_pruned(:, i-1) + k * constant_term;
end
